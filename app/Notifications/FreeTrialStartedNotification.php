...
use App\Mail\FreeTrialCreatedMail;
use App\Models\App;
...
class FreeTrialStartedNotification extends Notification
{
    use Queueable;
    ...
    public function toMail(object $notifiable): MailMessage
    {
        ...
        if ($this->app === App::BZR_KEY) {
            $emailView = 'emails.bzr.freeTrialStarted';
        } else if ($this->app === App::EI_KEY) {
            $emailView = 'emails.ei.freeTrialStarted';
        } else if ($this->app === App::ZZS_KEY) {
            $emailView = 'emails.zzs.freeTrialStarted';
        }
        ...
        /* return (new FreeTrialCreatedMail($notifiable))
            ->from('office@actamedia.rs')
            ->subject('Probni period')
            ->to($notifiable->email);  */
    }
